...
[CustomEditor(typeof(LocationGraph))]
public class GraphEditor : Editor
{
    ...
    {
        foreach(Location location in graph.vertices)
        {
            Vector3 worldPos;
    ...
    private void AddLocation()
    {
        ...
        bool button = GUILayout.Button("Add node");
        EditorGUIUtility.labelWidth = 50;
        value = EditorGUILayout.IntField("Label: ", value, GUILayout.ExpandWidth(false));
        if (button)
        {
            //graph.Add(new Location(value, Vector3.zero));
            graph.Add(Location.Create(value, Vector3.zero));
        }
    ...
    public override void OnInspectorGUI()
    {
        ...
        /*if (showVertices)
        {
            ...
            { 
                ...
                EditorGUILayout.LabelField("Location " + location.label, GUILayout.Width(size));
                EditorGUILayout.EndVertical();

                EditorGUILayout.BeginVertical();

                location.position = EditorGUILayout.Vector3Field("Position", location.position);
